...
imagesRouter.put('/set-main-image', async (req, res) => {
    ...
    if (image.includes('_MAIN')) {
        ...
        if (match) {
            ...
            let newPath = path.join(IMAGES_DIR, newName);
            ...
            const originalPath = path.join(IMAGES_DIR, originalName);
        ...
        const imagePath = path.join(IMAGES_DIR, originalName);
        const newImagePath = path.join(IMAGES_DIR, newName);
